...
//  if(fabs(vtx_z) > 15.)return;
  ...
  float leading_PFenergy = 0;
	...
	int type = cand.particleId();
  	...
  	const bool eta_pluspf = (cand.eta() > 3.0) && (cand.eta() < 6.0);
		...
		pf_energy->Fill(cand.energy());
                if(cand.energy()>=leading_PFenergy){leading_PFenergy = cand.energy();}
		countpf=countpf+1;
	}
        if(eta_minuspf){
    ...
    if(std::min(count_TH4_plus, count_TH4_minus) < 2){evfaily=evfaily+1;}else{evpassy=evpassy+1;}
